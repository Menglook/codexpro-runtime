...
import { findIntroducedSecretValues, findSecretValues, redactSensitiveText, type SecretValueFinding } from "./redact.js";
...
function secretWriteError(action: "write" | "edit", relPath: string, finding: SecretValueFinding): CodexProError {
  return new CodexProError(
    `Secret-looking content is blocked from ${action} at ${relPath}:${finding.line}:${finding.column} (${finding.rule}). ` +
    `${finding.message}. Use [REDACTED_SECRET] or an environment-variable reference. ` +
    `Synthetic fixtures may use "codexpro-secret-scan: allow-test-vector" only in a test/fixture path and must contain a fake/test/example marker.`
  );
}

function assertNoSecretWrite(action: "write" | "edit", relPath: string, content: string): void {
  const finding = findSecretValues(content, { path: relPath })[0];
  if (finding) throw secretWriteError(action, relPath, finding);
}

function assertNoIntroducedSecretWrite(action: "write" | "edit", relPath: string, before: string, after: string): void {
  const finding = findIntroducedSecretValues(before, after, { path: relPath })[0];
  if (finding) throw secretWriteError(action, relPath, finding);
}
...
export async function listFiles(
  ...
  function isExcluded(rel: string): boolean {
    return Boolean(options.excludeGlobs?.some((glob) => minimatch(rel, glob, { dot: true })));
  }
  ...
  async function walk(absDir: string): Promise<void> {
    if (files.length >= options.maxFiles || options.shouldStop?.()) return;
...
async function readLargeTextRange(
  absPath: string,
  relPath: string,
  startLine: number,
  ...
  input.on("data", (chunk: string | Buffer) => {
    ...
    if (bytesChunk.length) lastByte = bytesChunk[bytesChunk.length - 1] ?? null;
  });

  const appendSelected = (line: string) => {
    selectedBytes += Buffer.byteLength(line, "utf8") + (selected.length ? 1 : 0);
    if (selectedBytes > maxBytes) {
      throw new CodexProError(`Requested line range is too large (${selectedBytes} bytes). Limit: ${maxBytes} bytes.`);
    }
    selected.push(line);
  };
  ...
  try {
    for await (const line of reader) {
      totalLines += 1;
      if (totalLines >= startLine && totalLines <= requestedEndLine) appendSelected(line);
    }
  } catch (error) {
    input.destroy();
    throw error;
  ...
  if (bytes === 0) {
    totalLines = 1;
    if (startLine <= 1 && requestedEndLine >= 1) appendSelected("");
  } else if (lastByte === 0x0a) {
    totalLines += 1;
    if (totalLines >= startLine && totalLines <= requestedEndLine) appendSelected("");
  }
  ...
  return {
    path: relPath,
    text: withLineNumbers(selected, startLine),
    startLine,
    endLine,
    totalLines,
    bytes,
...
export async function readTextFile(
  ...
  await guard.assertTextFile(resolved.absPath, maxBytes, { allowLarger: hasExplicitRange });
  const stat = await fsp.stat(resolved.absPath);
  const startLine = Math.max(1, Math.floor(options.startLine ?? 1));
  const requestedEndLine = Math.floor(options.endLine ?? Number.MAX_SAFE_INTEGER);
  if (requestedEndLine < startLine) {
    throw new CodexProError(`end_line (${requestedEndLine}) must be >= start_line (${startLine}).`);
  }
  if (stat.size > maxBytes && hasExplicitRange) {
    return await readLargeTextRange(
      resolved.absPath,
      resolved.relPath,
      startLine,
      requestedEndLine,
...
function isInternalArtifactPath(config: CodexProConfig, relPath: string): boolean {
  const normalized = relPath.replace(/\\/g, "/").replace(/^\.\//, "");
  const contextDir = config.contextDir.replace(/\\/g, "/").replace(/^\.\//, "").replace(/\/$/, "");
  return normalized === contextDir
    || normalized.startsWith(`${contextDir}/`)
    || normalized === ".codexpro/runs"
    || normalized.startsWith(".codexpro/runs/");
}

async function withWorkspaceWriteLease<T>(
  config: CodexProConfig,
  workspace: Workspace,
  relPath: string,
  existingLease: WorkspaceLease | undefined,
  operation: (lease?: WorkspaceLease) => Promise<T>
): Promise<T> {
  if (isInternalArtifactPath(config, relPath)) return await operation(existingLease);
  let lease = existingLease;
  let owned = false;
  if (lease) {
    lease = heartbeatWorkspaceLeaseSync(workspace.root, lease, { contextDir: config.contextDir, name: "write" });
  } else {
    lease = acquireWorkspaceLeaseSync(workspace.root, {
  ...
  try {
    ...
    if (owned) releaseWorkspaceLeaseSync(workspace.root, lease, { contextDir: config.contextDir, name: "write" });
  }
}
...
export async function writeTextFile(
  ...
  filePath: string,
  content: string,
  options: { createDirs?: boolean; overwrite?: boolean; workspaceLease?: WorkspaceLease } = {}
  ...
  return await withWorkspaceWriteLease(config, workspace, resolved.relPath, options.workspaceLease, async () => {
    ...
    if (existed) assertNoIntroducedSecretWrite("write", resolved.relPath, oldText, content);
    else assertNoSecretWrite("write", resolved.relPath, content);
...
export async function editTextFile(
  ...
  oldText: string,
  newText: string,
  options: { replaceAll?: boolean; expectedReplacements?: number; workspaceLease?: WorkspaceLease } = {}
  ...
  return await withWorkspaceWriteLease(config, workspace, resolved.relPath, options.workspaceLease, async () => {
    ...
    assertNoIntroducedSecretWrite("edit", resolved.relPath, before, after);

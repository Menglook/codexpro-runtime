...
<script>
  ...
  function titleFor(tool) {
    const titles = {
      ...
      git_prepare_commit: "Prepare exact commit",
      git_commit: "Create commit",
      git_get_remote_state: "Git remote state",
      git_push: "Push exact commit",
      git_prepare: "Git Prepare",
      git_finalize: "Git Finalize",
      git_push_only: "Git Push Only",
  ...
  function iconFor(tool) {
    ...
    if (["git_status", "git_diff", "git_summary", "git_prepare_commit", "git_commit", "git_get_remote_state", "git_push", "git_prepare", "git_finalize", "git_push_only"].includes(tool)) return "G";
  ...
  function subtitleFor(data) {
    ...
    if (["git_status", "git_summary", "git_prepare_commit", "git_commit", "git_get_remote_state", "git_push", "git_prepare", "git_finalize", "git_push_only"].includes(data?.codexpro_tool)) {
      const count = Array.isArray(data?.changed_files) ? data.changed_files.length : 0;
      if (data?.codexpro_tool === "git_prepare" && data?.commit_flow_allowed === false) return "Commit flow blocked";
      if (data?.codexpro_tool === "git_prepare" && data?.commit_flow_allowed === true) return "Commit flow allowed";

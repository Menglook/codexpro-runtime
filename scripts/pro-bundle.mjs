...
async function main() {
  ...
  if (args.copy) {
    const copied = runManagedProcessSync('pbcopy', [], {
      stdin: result.markdown,
      maxOutputBytes: 64 * 1024,
      domain: 'shell',
      operation: 'pro_bundle_pbcopy',
      sideEffectLevel: 'external_write',
      riskLevel: 'low',
      recordRoot: workspace.root
    });
    if (copied.exitCode !== 0) {
      throw new Error(`pbcopy failed: ${copied.stderr || copied.errorClass || `exit ${copied.exitCode}`}`);
    }

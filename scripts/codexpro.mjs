...
} from './lib/handoff-execution.mjs';
import {
  HANDOFF_EXECUTION_TIMEOUT_DEFAULT_MS,
  formatHandoffExecutionTimeout,
...
Options:
  ...
  --tool-mode <minimal|progressive|standard|full>
                             Tool surface exposed to ChatGPT. Default: progressive.
  ...
  --widget-domain <origin>   Dedicated HTTPS origin for ChatGPT widget iframes.
                             Required for app submission. Default: https://example.github.io.
...
Execute handoff options:
  ...
  --timeout-ms <ms>         Handoff absolute execution limit. Default: ${formatHandoffExecutionTimeout(HANDOFF_EXECUTION_TIMEOUT_DEFAULT_MS)}.
                             Priority: CLI > .codexpro/project.yml > CODEXPRO_HANDOFF_EXECUTION_TIMEOUT_MS > built-in default.
  --max-output-bytes <n>    Max stdout/stderr excerpt bytes per stream. Default: 120000.
  --recovery-from-run-id <id>
                             Mark this run as a recovery attempt for a prior Handoff run. The old run and worktree are preserved.
...
Watch handoff options:
  ...
  --timeout-ms <ms>         Handoff absolute execution limit for each executed plan.
  --recovery-from-run-id <id>
                             Mark executed plans as recovery attempts for a prior Handoff run.
...
Loop handoff options:
  ...
  --timeout-ms <ms>         Handoff executor absolute execution limit for each iteration.
...
function commandExists(command) {
  const result = runManagedProcessSync(process.platform === 'win32' ? 'where' : 'command', process.platform === 'win32' ? [command] : ['-v', command], {
    shell: process.platform !== 'win32',
    stdio: 'ignore',
    timeoutMs: 2000,
    maxOutputBytes: 8_000,
    domain: 'probe',
    operation: 'command_exists',
...
function verifyCloudflared(binaryPath) {
  const result = runManagedProcessSync(binaryPath, ['--version'], {
    stdio: 'ignore',
    shell: false,
    timeoutMs: 15000,
    domain: 'probe',
    operation: 'verify_cloudflared',
    sideEffectLevel: 'none',
    riskLevel: 'low',
    recordRoot: process.cwd()
  });
...
async function installCloudflaredLocal() {
  ...
  try {
    if (asset.archive) {
      ...
      const tar = runManagedProcessSync('tar', ['-xzf', archivePath, '-C', extractDir], {
        stdio: ['ignore', 'pipe', 'pipe'],
        shell: false,
        maxOutputBytes: 512 * 1024,
        domain: 'shell',
        operation: 'extract_cloudflared',
        sideEffectLevel: 'local_write',
        riskLevel: 'medium',
        recordRoot: process.cwd()
      });
      if (tar.exitCode !== 0) {
        throw new Error(`Failed to extract ${asset.file}: ${tar.stderr || tar.stdout || `exit ${tar.exitCode}`}`);
      }
...
function runHelperScript(scriptName, args) {
  ...
  const result = runManagedProcessSync(process.execPath, [scriptPath, ...args], {
    cwd: projectRoot,
    env: { ...process.env, CODEXPRO_CALLER_CWD: process.cwd() },
    stdio: 'inherit',
    domain: 'shell',
    operation: `helper_${scriptName}`,
    sideEffectLevel: 'local_write',
    riskLevel: 'medium',
    recordRoot: process.cwd()
  });
  if (result.spawnError) throw new Error(result.stderr || result.errorClass || 'helper spawn failed');
...
function spawnLogged(name, command, args, options = {}) {
  ...
  const started = startManagedProcess(command, args, {
    ...spawnOptions,
    stdio: ['ignore', 'pipe', 'pipe'],
    captureOutput: true,
    onStdout: (chunk) => record(process.stdout, chunk),
    onStderr: (chunk) => record(process.stderr, chunk),
    domain: name.includes('tunnel') || name.includes('cloudflare') || name.includes('ngrok') ? 'worker' : 'server',
    operation: name,
    sideEffectLevel: 'local_write',
    riskLevel: 'medium',
    recordRoot: spawnOptions.cwd || process.cwd()
  });
  const child = started.child;
  if (!child) throw new Error(`Failed to start ${name}: ${started.errorClass ?? 'spawn failed'}`);
...
function copyToClipboard(text) {
  ...
  for (const [command, args] of attempts) {
    ...
    if (!exists) continue;
    const result = runManagedProcessSync(command, args, {
      stdin: text,
    ...
    if (result.exitCode === 0) return { ok: true, command };
  }
...
function openUrl(url) {
  ...
  const result = runManagedProcessSync(bin, args, {
    stdio: 'ignore',
    shell: false,
    timeoutMs: 10_000,
    maxOutputBytes: 8_000,
...
function loadHandoffExecution(args) {
  ...
  const timeoutResolution = resolveHandoffExecutionTimeoutMs({
    cliValue: args.timeoutMs ?? args.timeout,
    configValue: readProjectHandoffExecutionTimeoutConfig(root),
  ...
  const recoveryFromRunId = normalizeHandoffRunId(
    args.recoveryFromRunId ?? args.resumeFromRunId ?? process.env.CODEXPRO_HANDOFF_RECOVERY_FROM_RUN_ID,
    '--recovery-from-run-id'
  );
  return {
    root,
    contextDir,
    ...
    timeoutSource: timeoutResolution.sourceLabel,
    recoveryFromRunId
  };
}

function normalizeHandoffRunId(value, label) {
  if (value === undefined || value === null || value === '') return undefined;
  const text = String(value).trim();
  if (!text) return undefined;
  if (!/^[A-Za-z0-9._:-]{1,128}$/.test(text)) {
    throw new Error(`${label} must be 1-128 characters using letters, numbers, dot, underscore, colon, or dash.`);
  }
  return text;
}

function readHandoffRunStateById(root, contextDir, runId) {
  if (!runId) return {};
  const runStatePath = resolveWorkspaceFile(root, path.posix.join(contextDir, 'runs', runId, 'state.json'));
  try {
    const parsed = JSON.parse(fs.readFileSync(runStatePath, 'utf8'));
    return parsed && typeof parsed === 'object' && parsed.run_id === runId ? parsed : {};
  } catch {
    return {};
  }
}
...
function printHandoffDryRun(request, title = 'CodexPro execute-handoff dry run') {
  printBox(title, [
    ...
    labelValue('Timeout', `${formatHandoffExecutionTimeout(request.timeoutMs)} from ${request.timeoutSource}`),
    ...(request.recoveryFromRunId ? [labelValue('Recovery', `from run ${request.recoveryFromRunId}`)] : []),
...
async function executeHandoffRequest(request, args, options = {}) {
  ...
  const noProgressTimeoutMs = numberOption(options.noProgressTimeoutMs ?? args.noProgressTimeoutMs, Math.min(30 * 60_000, request.timeoutMs), 1, request.timeoutMs);
  const takeoverTimeoutMs = numberOption(options.takeoverTimeoutMs ?? args.takeoverTimeoutMs, 2 * 60_000, 1_000, 60 * 60_000);
  const killGraceMs = numberOption(options.killGraceMs ?? args.killGraceMs, 2_000, 100, 60_000);
  const restartCount = Math.max(0, Math.floor(Number(options.restartCount ?? 0)));
  const sameFailureRepeats = Math.max(0, Math.floor(Number(options.sameFailureRepeats ?? 0)));
  const maxAutoRestarts = Math.max(0, Math.floor(Number(options.maxAutoRestarts ?? args.maxAutoRestarts ?? 1)));
  const maxSameFailureRepeats = Math.max(1, Math.floor(Number(options.maxSameFailureRepeats ?? args.maxSameFailureRepeats ?? 2)));
  const runPlanHash = planHash(request.planText);
  const startedAt = new Date().toISOString();
  const startedAtMs = Date.parse(startedAt);
  const identity = createRunIdentity(`handoff:${request.commandInfo.agent}`);
  const previousRecoveryState = readHandoffRunStateById(request.root, request.contextDir, request.recoveryFromRunId);
  const previousResumeCount = Number.isFinite(Number(previousRecoveryState.resume_count))
    ? Math.max(0, Math.floor(Number(previousRecoveryState.resume_count)))
    : 0;
  const resumeCount = request.recoveryFromRunId
    ? previousResumeCount + 1
    : 0;
  let lease = acquireWorkspaceLeaseSync(request.root, {
  ...
  const runDir = createRunDirectorySync(request.root, request.contextDir, identity.runId);
  const runRelative = path.relative(request.root, runDir).replace(/\\/g, '/');
  const baseState = {
    run_id: identity.runId,
    [HANDOFF_OWNER_FIELD]: identity.ownerId,
    [HANDOFF_FENCING_FIELD]: lease.fencing_token,
    run_dir: runRelative,
    iteration,
    ...
    execution_timeout_ms: request.timeoutMs,
    no_progress_timeout_ms: noProgressTimeoutMs,
    takeover_timeout_ms: takeoverTimeoutMs,
    kill_grace_ms: killGraceMs,
    hard_deadline: new Date(startedAtMs + request.timeoutMs).toISOString(),
    no_progress_deadline: new Date(startedAtMs + noProgressTimeoutMs).toISOString(),
    takeover_deadline: new Date(startedAtMs + takeoverTimeoutMs).toISOString(),
    timeout_reason: null,
    duration_ms: 0,
    last_output_at: null,
    termination_signal: null,
    recovery_from_run_id: request.recoveryFromRunId,
    resume_count: resumeCount,
    restart_count: restartCount,
    same_failure_repeats: sameFailureRepeats,
    max_auto_restarts: maxAutoRestarts,
    max_same_failure_repeats: maxSameFailureRepeats
  };
  let currentExecutorPid = process.pid;
  let lastOutputAt;
  let lastRunningStateWriteAt = 0;
  let firstProgressLogged = false;
  const runningState = () => ({
    ...baseState,
    state: 'running',
    finished_at: null,
    pid: currentExecutorPid,
    duration_ms: Math.max(0, Date.now() - startedAtMs),
    last_output_at: lastOutputAt ?? null,
    no_progress_deadline: new Date(Date.parse(lastOutputAt ?? startedAt) + noProgressTimeoutMs).toISOString()
  });
  const writeRunningState = (force = false) => {
    const now = Date.now();
    if (!force && now - lastRunningStateWriteAt < 1000) return;
    lastRunningStateWriteAt = now;
    writeHandoffRunState(request.root, request.contextDir, runningState());
  };
  writeHandoffRunState(request.root, request.contextDir, {
    ...runningState(),
    pid: process.pid
  }, { replaceOwner: true });
  appendBridgeLog(request.root, request.contextDir, {
    event: 'run_created',
    source: 'handoff_executor',
    correlation_id: identity.runId,
    run_id: identity.runId,
    task_id: `handoff-${runPlanHash}`,
    plan_hash: runPlanHash,
    owner_token: identity.ownerId,
    fencing_token: lease.fencing_token,
    previous_owner_token: previousRecoveryState?.[HANDOFF_OWNER_FIELD] ?? null,
    timestamp: startedAt,
    started_at: startedAt
  });
  appendBridgeLog(request.root, request.contextDir, {
  ...
  try {
    ...
    const result = await runProcessCaptured(request.commandInfo.command, request.commandInfo.args, {
      ...
      redact: redactForLog,
      onSpawn: (pid) => {
        currentExecutorPid = pid;
        lease = heartbeatWorkspaceLeaseSync(request.root, lease, {
          contextDir: request.contextDir,
          name: 'write',
          managedPid: pid
        });
      ...
      onOutput: (event) => {
        lastOutputAt = typeof event?.at === 'string' ? event.at : new Date().toISOString();
        ...
        if (!firstProgressLogged) {
          ...
          appendBridgeLog(request.root, request.contextDir, {
            ...
            run_id: identity.runId,
            task_id: `handoff-${runPlanHash}`,
            progress_at: lastOutputAt,
            liveness_at: new Date().toISOString(),
            progress_fingerprint: `handoff-output:${identity.runId}`,
            timestamp: lastOutputAt
          });
        }
      },
      onHeartbeat: () => {
        lease = heartbeatWorkspaceLeaseSync(request.root, lease, {
          contextDir: request.contextDir,
          name: 'write',
          managedPid: lease.managed_pid
        });
        writeRunningState(false);
        return options.onHeartbeat?.();
      }
    });
    ...
    const outputs = writeExecutionOutputs(request.root, request.contextDir, request.commandInfo, result, diffText, {
      runId: identity.runId,
      ownerId: identity.ownerId,
      ["fencing" + "Token"]: lease.fencing_token,
      runDir,
      executionTimeoutMs: request.timeoutMs,
      recoveryFromRunId: request.recoveryFromRunId,
      resumeCount
    });
    const runState = result.cancelled
      ? 'cancelled'
      : result.timedOut
        ? 'timed_out'
        : (result.exitCode === 0 && result.treeTerminated ? 'completed' : 'failed');
    ...
    writeHandoffRunState(request.root, request.contextDir, {
      ...
      termination_requested_at: result.terminationRequestedAt ?? null,
      termination_force_used: Boolean(result.forceUsed),
      heartbeat_write_failures: Number(result.heartbeatFailures) || 0,
      timed_out: Boolean(result.timedOut),
      cancelled: Boolean(result.cancelled),
      tree_terminated: Boolean(result.treeTerminated),
      duration_ms: result.durationMs,
      last_output_at: lastOutputAt ?? result.lastProgressAt ?? null,
      no_progress_deadline: new Date(Date.parse(lastOutputAt ?? result.lastProgressAt ?? startedAt) + noProgressTimeoutMs).toISOString(),
      status_file: path.relative(request.root, outputs.statusPath).replace(/\\/g, '/'),
      diff_file: path.relative(request.root, outputs.diffPath).replace(/\\/g, '/'),
      log_file: path.relative(request.root, outputs.logPath).replace(/\\/g, '/'),
      compatibility_status_file: path.posix.join(request.contextDir, 'agent-status.md'),
      compatibility_diff_file: path.posix.join(request.contextDir, 'implementation-diff.patch'),
    ...
    appendBridgeLog(request.root, request.contextDir, {
      ...
      liveness_at: new Date().toISOString(),
      progress_fingerprint: `handoff-terminal:${runState}`,
      timestamp: new Date().toISOString()
    });
    statusLine(result.exitCode === 0 && result.treeTerminated ? 'ok' : 'warn', `Agent exited with code ${result.exitCode ?? 'null'}${result.signal ? ` signal=${result.signal}` : ''}`);
    console.log(`Run:    ${runRelative}`);
    ...
    return { cancelled: false, result, outputs, runId: identity.runId };
  } catch (error) {
    writeHandoffRunState(request.root, request.contextDir, {
      ...baseState,
      state: 'failed',
      finished_at: new Date().toISOString(),
      duration_ms: Math.max(0, Date.now() - startedAtMs),
      timeout_reason: error?.code === 'WORKSPACE_WRITE_LEASE_BUSY' ? 'resource_limit' : 'process_exit',
      termination_signal: null,
      last_output_at: lastOutputAt ?? null,
    ...
    appendBridgeLog(request.root, request.contextDir, {
      ...
      owner_token: identity.ownerId,
      fencing_token: lease.fencing_token,
      owner_pid: currentExecutorPid,
      timestamp: new Date().toISOString()
    });
    releaseWorkspaceLeaseSync(request.root, lease, { contextDir: request.contextDir, name: 'write' });
  }
}
...
function writeWatchState(statePath, state) {
  atomicWriteJsonSync(statePath, { version: 2, updated_at: new Date().toISOString(), ...state });
}

function watcherHeartbeatPath(root, contextDir) {
  return resolveWorkspaceFile(root, path.posix.join(contextDir, 'watch-handoff-heartbeat.json'));
}

function readCurrentHandoffRunState(root, contextDir) {
  try {
...
function terminalHandoffState(state) {
  return ['completed', 'failed', 'timed_out', 'cancelled', 'blocked'].includes(String(state ?? ''));
}

function handoffFailureFingerprint(result) {
  return createHash('sha256').update(JSON.stringify({
    exitCode: result?.exitCode ?? null,
    signal: result?.signal ?? null,
    timedOut: Boolean(result?.timedOut),
    cancelled: Boolean(result?.cancelled),
    treeTerminated: result?.treeTerminated !== false,
    terminationReason: classifyHandoffTerminationReason(result),
    stderr: String(result?.stderr ?? '').slice(-2000)
  })).digest('hex');
}

function writeWatcherHeartbeat(root, contextDir, lease, identity, args, state = {}) {
  const heartbeatPath = watcherHeartbeatPath(root, contextDir);
  const previous = readWatchState(heartbeatPath);
  const now = new Date().toISOString();
  const leaseStatus = readWorkspaceLeaseSync(root, { contextDir, name: 'watcher' });
  const healthProbeSeq = Math.max(0, Number(previous.health_probe_seq) || 0) + 1;
  atomicWriteJsonSync(heartbeatPath, {
    version: 3,
    root,
    ...
    watcher_run_id: identity.runId,
    [HANDOFF_OWNER_FIELD]: identity.ownerId,
    [HANDOFF_FENCING_FIELD]: lease?.fencing_token ?? null,
    lease_active: leaseStatus.status === 'active',
    lease_expires_at: lease?.expires_at ?? null,
    health_probe_at: now,
    health_probe_seq: healthProbeSeq,
    health_probe_status: leaseStatus.status === 'active' && isProcessAlive(process.pid) ? 'ok' : 'degraded',
...
function reconcileInterruptedHandoff(root, contextDir, noProgressTimeoutMs) {
  const previous = readCurrentHandoffRunState(root, contextDir);
  if (terminalHandoffState(previous.state) || previous.state !== 'running') return previous;
  const pidAlive = isProcessAlive(Number(previous.pid));
  const lastProgressAt = Date.parse(previous.last_output_at ?? previous.started_at ?? '');
  const noProgressExpired = Number.isFinite(lastProgressAt) && Date.now() - lastProgressAt >= noProgressTimeoutMs;
  if (pidAlive && !noProgressExpired) return previous;

  const identity = createRunIdentity('handoff-reconcile');
  const lease = acquireWorkspaceLeaseSync(root, {
    contextDir,
    name: 'write',
    kind: 'handoff-reconcile',
    runId: identity.runId,
    ownerId: identity.ownerId,
    pid: process.pid,
    ttlMs: 30_000
  ...
  const now = new Date().toISOString();
  const blockedReason = pidAlive
    ? 'Previous Handoff exceeded its no-progress deadline; automatic replay is blocked pending reconciliation.'
    : 'Previous Handoff executor is no longer alive; automatic replay is blocked pending reconciliation.';
  try {
    const blocked = {
      ...previous,
      run_id: identity.runId,
      recovery_from_run_id: previous.run_id ?? null,
      [HANDOFF_OWNER_FIELD]: identity.ownerId,
      [HANDOFF_FENCING_FIELD]: lease.fencing_token,
      state: 'blocked',
      started_at: previous.started_at ?? now,
      finished_at: now,
      duration_ms: Math.max(0, Date.now() - Date.parse(previous.started_at ?? now)),
      timeout_reason: noProgressExpired ? 'no_progress_timeout' : 'process_exit',
      blocked_reason: blockedReason,
      error: blockedReason,
      pid: process.pid,
      tree_terminated: !pidAlive
    };
    writeHandoffRunState(root, contextDir, blocked, { replaceOwner: true });
    appendBridgeLog(root, contextDir, {
      event: 'handoff_reconciled_blocked',
      run_id: identity.runId,
      recovery_from_run_id: previous.run_id ?? null,
      fencing_token: lease.fencing_token,
    ...
    releaseWorkspaceLeaseSync(root, lease, { contextDir, name: 'write' });
  }
}
...
function writeHandoffRunState(root, contextDir, state, options = {}) {
  const statePath = handoffRunStatePath(root, contextDir);
  const payload = { version: 2, updated_at: new Date().toISOString(), ...state };
  if (state.run_dir) {
    const runStatePath = resolveWorkspaceFile(root, path.posix.join(state.run_dir, "state.json"));
    atomicWriteOwnedJsonSync(runStatePath, payload, {
      ownerId: state[HANDOFF_OWNER_FIELD],
      ["fencing" + "Token"]: state[HANDOFF_FENCING_FIELD],
      replaceOwner: options.replaceOwner
    });
  }
  atomicWriteOwnedJsonSync(statePath, payload, {
    ownerId: state[HANDOFF_OWNER_FIELD],
    ["fencing" + "Token"]: state[HANDOFF_FENCING_FIELD],
    replaceOwner: options.replaceOwner
  });
...
function appendBridgeLog(root, contextDir, event) {
  ...
  appendJsonLineSync(logPath, { version: 2, ts: new Date().toISOString(), ...event });
}
...
async function runWatchHandoff(argv) {
  ...
  let lastSkippedHash = '';
  let stopped = false;
  const watcherNoProgressTimeoutMs = numberOption(args.noProgressTimeoutMs, 30 * 60_000, 100, 24 * 60 * 60_000);
  const watcherTakeoverTimeoutMs = numberOption(args.takeoverTimeoutMs, 2 * 60_000, 1_000, 60 * 60_000);
  const maxAutoRestarts = Math.max(0, Math.floor(Number(args.maxAutoRestarts ?? 1)));
  const maxSameFailureRepeats = Math.max(1, Math.floor(Number(args.maxSameFailureRepeats ?? 2)));
  let pendingRecoveryFromRunId = null;
  let restartCount = Math.max(0, Math.floor(Number(state.restartCount ?? 0)));
  let sameFailureRepeats = Math.max(0, Math.floor(Number(state.sameFailureRepeats ?? 0)));
  let lastFailureFingerprint = String(state.lastFailureFingerprint ?? '');
  ...
  const watcherIdentity = createRunIdentity('handoff-watcher');
  let watcherLease = acquireWorkspaceLeaseSync(root, {
    contextDir,
    name: 'watcher',
    kind: 'handoff-watcher',
    runId: watcherIdentity.runId,
    pid: process.pid,
    ttlMs: 60_000
  });
  const previousRunState = reconcileInterruptedHandoff(root, contextDir, watcherNoProgressTimeoutMs);
  if (terminalHandoffState(previousRunState.state) && previousRunState.plan_hash) {
    state.lastPlanHash = state.lastPlanHash || previousRunState.plan_hash;
  }
  writeWatcherHeartbeat(root, contextDir, watcherLease, watcherIdentity, args, {
    state: previousRunState.state === "blocked" ? "blocked" : "watching",
    blocked_reason: previousRunState.blocked_reason ?? null,
    restart_count: restartCount,
    same_failure_repeats: sameFailureRepeats,
    max_auto_restarts: maxAutoRestarts,
    max_same_failure_repeats: maxSameFailureRepeats
  ...
  printBox('CodexPro watch-handoff', [
    ...
    labelValue('Watcher run', watcherIdentity.runId),
  ...
  const stop = () => {
    stopped = true;
    watcherAbort.abort();
    statusLine('warn', 'Stopping handoff watcher and its managed process tree...');
  };
  ...
  try {
    while (!stopped) {
      watcherLease = heartbeatWorkspaceLeaseSync(root, watcherLease, { contextDir, name: 'watcher' });
      writeWatcherHeartbeat(root, contextDir, watcherLease, watcherIdentity, args, {
        state: state.blockedReason ? "blocked" : "watching",
      ...
      if (pendingRecoveryFromRunId) request.recoveryFromRunId = pendingRecoveryFromRunId;
      ...
      appendBridgeLog(root, contextDir, {
        ...
        watcher_run_id: watcherIdentity.runId,
      ...
      const execution = await executeHandoffRequest(request, { ...args, yes: true }, {
        skipConfirmation: true,
        signal: watcherAbort.signal,
        noProgressTimeoutMs: watcherNoProgressTimeoutMs,
        takeoverTimeoutMs: watcherTakeoverTimeoutMs,
        restartCount,
        sameFailureRepeats,
        maxAutoRestarts,
        ...
        onHeartbeat: () => {
          ...
          writeWatcherHeartbeat(root, contextDir, watcherLease, watcherIdentity, args, {
            ...
            restart_count: restartCount,
            same_failure_repeats: sameFailureRepeats,
            active_handoff_run_id: readCurrentHandoffRunState(root, contextDir).run_id ?? null
          });
        }
      });
      const exitCode = execution.result?.exitCode ?? null;
      const succeeded = Boolean(execution.result && execution.result.exitCode === 0 && execution.result.treeTerminated);
      const failureFingerprint = succeeded ? '' : handoffFailureFingerprint(execution.result);
      sameFailureRepeats = succeeded
        ? 0
        : failureFingerprint === lastFailureFingerprint
          ? sameFailureRepeats + 1
          : 1;
      lastFailureFingerprint = failureFingerprint;
      const terminationReason = execution.result ? classifyHandoffTerminationReason(execution.result) : 'process_exit';
      const safeToRestart = Boolean(
        !succeeded
        && execution.result
        && execution.result.treeTerminated
        && !execution.result.cancelled
        && terminationReason !== 'termination_failed'
      );
      const retryAllowed = safeToRestart
        && restartCount < maxAutoRestarts
        && sameFailureRepeats < maxSameFailureRepeats;
      if (retryAllowed) {
        restartCount += 1;
        pendingRecoveryFromRunId = execution.runId ?? null;
      } else {
        pendingRecoveryFromRunId = null;
      }
      const blockedReason = !succeeded && !retryAllowed
        ? safeToRestart
          ? 'Handoff automatic restart budget or repeated-failure budget was exhausted.'
          : 'Handoff external state is uncertain; automatic replay is blocked pending reconciliation.'
        : null;
      state = {
        version: 3,
        watcherRunId: watcherIdentity.runId,
        ...(retryAllowed ? {} : { lastPlanHash: currentHash }),
      ...
      writeWatcherHeartbeat(root, contextDir, watcherLease, watcherIdentity, args, {
        state: retryAllowed ? 'recovering' : blockedReason ? 'blocked' : 'watching',
        blocked_reason: blockedReason,
        restart_count: restartCount,
        same_failure_repeats: sameFailureRepeats,
        active_handoff_run_id: execution.runId ?? null,
        recovery_from_run_id: pendingRecoveryFromRunId
      });
      appendBridgeLog(root, contextDir, {
        event: 'watch_handoff_finished',
        watcher_run_id: watcherIdentity.runId,
        handoff_run_id: execution.runId,
        plan_hash: currentHash,
        agent: request.commandInfo.agent,
        model: request.commandInfo.model || undefined,
        exit_code: exitCode,
        status_path: execution.outputs ? path.relative(root, execution.outputs.statusPath).replace(/\\/g, '/') : undefined,
        diff_path: execution.outputs ? path.relative(root, execution.outputs.diffPath).replace(/\\/g, '/') : undefined
      });
    ...
    releaseWorkspaceLeaseSync(root, watcherLease, { contextDir, name: 'watcher' });
  }
}
...
function runGitText(root, args, maxBytes) {
  const result = runManagedProcessSync('git', args, {
    cwd: root,
    maxOutputBytes: Math.max(maxBytes * 2, 1_000_000),
    shell: false,
    domain: 'git',
  ...
  if (result.exitCode !== 0) {
    const reason = result.stderr || result.stdout || `git ${args.join(' ')} exited ${result.exitCode}`;
...
function printCreateAppFields(details) {
  ...
  console.log('');
  if (details.token) {
    console.log('Authentication token is configured but intentionally not printed.');
  } else {
...
function printConnectorBlock(endpoint, token, options = {}) {
  ...
  console.log(`  Mode       ${modeTitle}  tools=${options.toolMode ?? 'progressive'}  write=${options.write ?? 'workspace'}  bash=${options.bash ?? 'safe'}`);
  ...
  return { ...details, copied, opened, mode, toolMode: options.toolMode ?? 'progressive' };
}
...
async function runDoctor(argv) {
  ...
  const toolMode = optionValue(args, profile, 'toolMode', ['CODEXPRO_TOOL_MODE'], 'progressive');
  ...
  record(['minimal', 'progressive', 'standard', 'full'].includes(toolMode) ? 'ok' : 'fail', 'Tool mode', ['minimal', 'progressive', 'standard', 'full'].includes(toolMode) ? toolMode : '--tool-mode must be minimal, progressive, standard, or full');
...
async function runSetupWizard(argv) {
  ...
  try {
    ...
    const toolMode = optionalChoice('tool-mode', optionValue(defaults, profile, 'toolMode', ['CODEXPRO_TOOL_MODE'], ''), ['minimal', 'progressive', 'standard', 'full']);
...
function saveSettingsFromArgs(root, args, profile) {
  ...
  const toolMode = optionalChoice('tool-mode', optionValue(args, profile, 'toolMode', ['CODEXPRO_TOOL_MODE'], profile.toolMode ?? ''), ['minimal', 'progressive', 'standard', 'full']);
...
async function main() {
  ...
  const port = String(optionValue(args, profile, 'port', ['CODEXPRO_PORT'], '8787'));
  const startupHealthTimeoutMs = Number(optionValue(
    args,
    profile,
    'startupHealthTimeoutMs',
    ['CODEXPRO_STARTUP_HEALTH_TIMEOUT_MS'],
    '60000'
  ));
  if (!Number.isInteger(startupHealthTimeoutMs) || startupHealthTimeoutMs < 15_000 || startupHealthTimeoutMs > 600_000) {
    throw new Error('--startup-health-timeout-ms must be an integer between 15000 and 600000.');
  }
  const bash = optionValue(args, profile, 'bash', ['CODEXPRO_BASH_MODE'], 'safe');
  const bashTranscript = bashTranscriptOption(args, profile);
  const codexSessions = codexSessionsOption(args, profile);
  ...
  const write = writeOption(args, profile, mode);
  const toolMode = optionValue(args, profile, 'toolMode', ['CODEXPRO_TOOL_MODE'], 'progressive');
  const widgetDomain = optionValue(args, profile, 'widgetDomain', ['CODEXPRO_WIDGET_DOMAIN'], 'https://example.github.io');
  ...
  validateChoice('tool-mode', toolMode, ['minimal', 'progressive', 'standard', 'full']);
  ...
  try {
    await waitForHealth(`${localBase}/healthz`, token, startupHealthTimeoutMs);
  } catch (error) {
    const tail = typeof server.codexproLogTail === 'function' ? server.codexproLogTail() : '';

...
Generic `write`, `edit`, and `apply_patch_bundle` operations scan the resulting file content before it is written.

- Blocked results identify the file, line, column, and matching rule without printing the matched value.
- Use `[REDACTED_SECRET]`, an environment-variable reference, or a local ignored configuration file for real credentials.
- Batch patch operations are evaluated per file. Safe operations continue, rejected operations are reported individually, and the bundle returns `ok`, `partial`, or `failed` status.
- Synthetic test vectors may be admitted only in test, fixture, or smoke-test paths. The same or previous line must contain `codexpro-secret-scan: allow-test-vector`, and the synthetic value must visibly include a marker such as `fake`, `test`, `example`, `dummy`, or `placeholder`.
- The test-vector directive is not valid in production paths and does not admit secret-shaped values without a synthetic marker. Never use it to hide a real credential.
